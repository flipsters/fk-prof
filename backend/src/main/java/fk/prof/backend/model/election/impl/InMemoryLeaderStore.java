  ...
  public synchronized void setLeader(BackendDTO.LeaderDetail leaderDetail) {
    if (leaderDetail == null) {
      logger.info(String.format("Removing backend node as leader. Node IP = %s",
          this.currentLeaderDetail == null ? "" : BackendDTOProtoUtil.leaderDetailCompactRepr(leaderDetail)));
    } else {
      logger.info(String.format("Setting backend leader. Node IP = %s", BackendDTOProtoUtil.leaderDetailCompactRepr(leaderDetail)));
    }
